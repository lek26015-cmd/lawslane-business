...
export function CourseCard({ course }: CourseCardProps) {
    return (
        <Link href={`/education/courses/${course.id}`} className="group block h-full">
                        ...
                        <div className="flex items-center gap-1">
                            <div className="flex">
                                {[1, 2, 3, 4, 5].map((star) => (
                                    <Star
                                        key={star}
                                        className={`w-4 h-4 ${star <= Math.round(course.rating) ? 'text-amber-400 fill-amber-400' : 'text-slate-200 fill-slate-200'}`}
                                    />
                                ))}
                            </div>
