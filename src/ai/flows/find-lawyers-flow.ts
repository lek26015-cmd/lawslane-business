...
5.  Return the response as a JSON object matching this schema: {"specialties": ["Specialty1", "Specialty2"]}
...
`;
  ...
  try {
    ...
    return FindLawyersOutputSchema.parse(parsed);
  } catch (error) {
    console.error("[FindLawyersFlow] Failed to parse AI response:", responseText, error);
    return { specialties: [] };
  }
}
